...
import { IconAnalytics, IconBackLeft, IconCanselModal, IconClose, IconComment, IconDate, IconDelete, IconDeleteComment, IconDislike, IconEays, IconEdit, IconErow, Iconfevarite, IconfevariteActive, IconLike, IconMessage, IconSettingDot, IconTime, IconWorld, IconWornoingDelete } from '@/icons/Icon'
...
import { useCommentDeleteMutation, useCommentsQuery, useRepliesQuery } from '@/redux/apiSlices/videoDetails/videoDetailsSlice'
...
const videodetails = () => {
    ...
    const [deleteCommentModalVisible, setDeleteCommentModalVisible] = React.useState(false)
    ...
    const { data: singleVideo, isLoading, refetch:singleVideoRefetch } = useMy_videos_detailsQuery({ id });
    ...
    const player = useVideoPlayer(singleVideo?.data?.type === 'link' ? "" : (singleVideo?.data?.video || ""), (player) => {
    ...
    useFocusEffect(
        useCallback(() => {
            if (playerRef.current && isPlayerInitialized.current && singleVideo?.data?.type !== 'link') {
                try {
    ...
    useEffect(() => {
        ...
        if (singleVideo?.data?.link && (singleVideo?.data.link.includes('youtube.com') || singleVideo?.data.link.includes('youtu.be'))) {
    ...
    return (
        ...
        <View style={tw`flex-1 bg-primary`}>
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                >
                    <View style={styles.modalContainer}>
                        <View style={tw`bg-primary rounded-t-3xl absolute bottom-0  w-full  mt-78 pb-12`}>
                            <View
                                style={tw`bg-secondary w-full h-16 rounded-t-3xl flex-row items-center justify-between p-5`}
                            >
                                <View></View>
                            ...
                            <ScrollView contentContainerStyle={tw`px-4 `} showsVerticalScrollIndicator={false}>
                                ...
                                <View style={tw`flex-row justify-center gap-3`}>
                                    <TouchableOpacity onPress={() => setDeleteModalVisible(false)} style={tw` w-2/6 py-5 border flex-row rounded-full justify-center border-primaryGray`}>
                                        <Text style={tw`text-sm font-poppinsMedium`}>Cancel</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity onPress={() => hendelDeleteVideo()} style={tw` w-2/6 bg-secondary  py-5 border flex-row rounded-full justify-center border-primaryGray`}>
                    ...
                    <View style={styles.modalContainer}>
                        <View style={tw`bg-primary rounded-t-3xl absolute bottom-0  w-full  mt-78 pb-12`}>
                            ...
                            >
                                ...
                                <TouchableOpacity onPress={() => setDeleteCommentModalVisible(false)}>
                                    <SvgXml xml={IconClose} />
                                </TouchableOpacity>
                            </View>
                            ...
                            <ScrollView contentContainerStyle={tw`px-4 `} showsVerticalScrollIndicator={false}>
                                <View style={tw` flex-col items-center justify-center pt-5`}>
                                    <SvgXml xml={IconWornoingDelete} />
                                    <Text style={tw`font-poppinsMedium text-lg text-secondary pt-5`}>Are you sure to delete this comment?</Text>
                                    <Text style={tw`font-poppinsMedium text-sm text-primaryGrayDeep pt-1 pb-7`}>This action cannot be undone.</Text>
                                </View>
                                <View style={tw`flex-row justify-center gap-3`}>
                                    <TouchableOpacity
