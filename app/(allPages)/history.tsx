...
const HistoryScreen = () => {
  ...
  const loadHistory = useCallback(
    async (pageNum = page, searchText = searchQuery) => {
      ...
      <TouchableOpacity
        onPress={() =>
          router.push({
            ...
            params: { id: video?.id, slug: video?.slug },
          })
        }
      >
        <View style={[tw`flex-row gap-4 mb-6`, { width: "100%" }]}>
          <Image
            style={[
              tw`rounded-xl`,
              { width: _Width * 0.35, height: _HIGHT * 0.1 },
            ]}
  ...
  return (
    ...
    >
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        {settingsVisible && (
          <View
            style={tw`absolute bg-primaryText p-6 right-5 w-72 top-40 z-20 shadow-lg rounded-lg`}
          >
            <TouchableOpacity
            ...
            <TouchableOpacity
              onPress={handle_pause_play_watch}
              style={tw`flex-row items-start gap-3 pt-3`}
            >
              {userInfo?.data?.pause_watch_history ? (
        ...
        <View style={tw`px-6 mb-3`}>
          <View
            style={tw`border border-primaryGray flex-row items-center bg-primaryText px-4 rounded-full`}
          >
        ...
        {isLoading && page === 1 ? (
          ...
          <FlatList
            ...
            ListFooterComponent={
              loadingMore ? (
                <ActivityIndicator color="#0000ff" style={tw`my-4`} />
              ) : null
            }
